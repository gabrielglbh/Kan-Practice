  ...
  ListDetailBloc() : super(ListDetailStateLoading()) {
    ...
    on<ListDetailEventLoading>((event, emit) async {
      try {
        ...
        final List<Word> pagination = await getIt<WordRepositoryImpl>()
            .getAllWordsFromList(event.list,
    ...
    on<ListDetailEventSearching>((event, emit) async {
      try {
        ...
        final List<Word> pagination = await getIt<WordRepositoryImpl>()
            .getWordsMatchingQuery(event.query, event.list,
    ...
    on<ListDetailUpdateName>((event, emit) async {
      emit(ListDetailStateLoading());
      final error = await getIt<ListRepositoryImpl>()
      ...
      if (error == 0) {
        ...
        final List<Word> lists = await getIt<WordRepositoryImpl>()
    ...
    on<ListDetailEventLoadUpPractice>((event, emit) async {
      try {
        final List<Word> allList =
            await getIt<WordRepositoryImpl>().getAllWordsFromList(event.list);
