...
class MigrationUtils {
  batchUpdateTestMode(Batch? batch, Test test) {
    ...
    if (test.kanjiLists == "Blitz") {
      testMode = 1;
    } else if (test.kanjiLists == "Recuerdo" || test.kanjiLists == "Remembrance"
        || test.kanjiLists == "Erinnerung") {
      testMode = 2;
    } else if (test.kanjiLists == "Números aleatorios" || test.kanjiLists == "Random Numbers"
        || test.kanjiLists == "Zufallszahlen") {
      testMode = 3;
    } else if (test.kanjiLists == "Menos %" || test.kanjiLists == "Less %"
    ...
    batch?.update(TestTableFields.testTable, {
      TestTableFields.testModeField: testMode
    }, where: "${TestTableFields.takenDateField}=?", whereArgs: [test.takenDate],
